...
//        new java.io.File(subpastaFotosWeb).mkdirs();
        ...
        try (java.io.PrintWriter writer = new java.io.PrintWriter(caminhoArquivo, "UTF-8");
            ...
            writer.println("        } else {");
            writer.println("          html += `    <div style=\"background:#eee;height:200px;display:flex;align-items:center;justify-content:center;color:#999;\">Sem Foto</div>`;");
            writer.println("        }");
            writer.println("        html += `  </div>`;");
            writer.println("        html += `  <div class=\"produto-info\">`;");
            writer.println("        html += `    <h3>${nome}</h3>`;");
            writer.println("        html += `    <p>Tamanho: ${tamanho}</p>`;");
            writer.println("        html += `    <p>CÓDIGO: ${codigo}</p>`;");
            writer.println("        html += `    <p class=\"preco\">R$ ${preco.toFixed(2).replace('.', ',')}</p>`;");
            writer.println("        html += `    <button onclick=\"adicionarAoCarrinho('${codigo}', '${nome.replace(/'/g, \"\\\\'\")}', ${preco})\">ADICIONAR AO CARRINHO</button>`;");
            writer.println("        html += `  </div>`;");
            writer.println("        html += `</div>`;");
            writer.println("      });");
            writer.println("      ");
