...
public class GerarSiteEstoque {
    ...
    public void gerarSiteEstoque() throws ClassNotFoundException, java.sql.SQLException, InterruptedException {
        ...
        try (java.io.PrintWriter writer = new java.io.PrintWriter(caminhoArquivo, "UTF-8");
            ...
            writer.println("        // ==========================================");
            writer.println("        // 🔥 LÓGICA DOS BADGES (NOVO + PROMOÇÃO)");
            writer.println("        // ==========================================");
            writer.println("        let isNovo = false;");
            writer.println("        if (produto.dataCriacao) {");
            writer.println("          const dataCriacao = new Date(produto.dataCriacao + 'T00:00:00'); // força horário local");
            writer.println("          const agora = new Date();");
            writer.println("          const diff = agora - dataCriacao;");
            writer.println("          const dias = diff / (1000 * 60 * 60 * 24);");
            writer.println("          isNovo = dias <= 30;");
            writer.println("          console.log(`🆕 ${nome}: ${produto.dataCriacao} -> ${dias.toFixed(1)} dias -> isNovo=${isNovo}`);");
            writer.println("        } else {");
            writer.println("          console.log(`⚠️ ${nome}: dataCriacao não disponível`);");
            writer.println("        }");
            writer.println("        let badges = [];");
            writer.println("        if (isNovo) badges.push('Novo');");
            writer.println("        if (preco > 100) badges.push('Promoção');");
            writer.println("        const badgeText = badges.join(' '); // Ex: 'Novo Promoção'");
            writer.println("        // ==========================================");
            ...
            writer.println("          palavrasChave += ' ' + badgeText.toLowerCase();");
            writer.println("        }");
            ...
            writer.println("          }");
            writer.println("          // 🔥 BADGE (se houver)");
            writer.println("          if (badgeText) {");
