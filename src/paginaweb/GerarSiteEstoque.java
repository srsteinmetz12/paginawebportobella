...
public class GerarSiteEstoque {
    ...
    public void gerarSiteEstoque() throws ClassNotFoundException, java.sql.SQLException, InterruptedException {
        ...
        try (java.io.PrintWriter writer = new java.io.PrintWriter(caminhoArquivo, "UTF-8");
            ...
            writer.println("            }");
            writer.println("");
            writer.println("            let promessas = itensParaReservar.map(item =>");
            ...
            writer.println("                })");
            writer.println("              }).then(res => res.json())");
            writer.println("            );");
            writer.println("");
            writer.println("            Promise.all(promessas)");
            writer.println("              .then(resultados => {");
            writer.println("                let reservados = [];");
            writer.println("                let falhas = [];");
            writer.println("                ");
            writer.println("                resultados.forEach((data, index) => {");
            writer.println("                  if (data.success) {");
            writer.println("                    reservados.push(itensParaReservar[index].id);");
            writer.println("                  } else {");
            writer.println("                    falhas.push(itensParaReservar[index].id);");
            writer.println("                  }");
            writer.println("                });");
            writer.println("");
            writer.println("                console.log('📊 [RESERVA] Reservados:', reservados);");
            writer.println("                console.log('📊 [RESERVA] Falhas:', falhas);");
            writer.println("");
            writer.println("                if (reservados.length > 0) {");
            writer.println("                  removerItensVitrine(reservados);");
            writer.println("                }");
            writer.println("");
            writer.println("                if (falhas.length > 0) {");
            writer.println("                  alert('⚠️ Alguns itens não puderam ser reservados:\\n\\n' +");
            writer.println("                        '✅ Reservados: ' + reservados.join(', ') + '\\n' +");
            writer.println("                        '❌ Indisponíveis: ' + falhas.join(', ') + '\\n\\n' +");
            writer.println("                        'Os indisponíveis foram removidos do seu carrinho.');");
            writer.println("                  ");
            writer.println("                  carrinho.itens = carrinho.itens.filter(item => !falhas.includes(item.id));");
            writer.println("                  atualizarCarrinho();");
            writer.println("                }");
            writer.println("");
