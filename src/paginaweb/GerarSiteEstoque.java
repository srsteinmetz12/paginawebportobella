...
public class GerarSiteEstoque {
    ...
    public void gerarSiteEstoque() throws ClassNotFoundException, java.sql.SQLException, InterruptedException {
        ...
        try (java.io.PrintWriter writer = new java.io.PrintWriter(caminhoArquivo, "UTF-8");
            ...
            writer.println("        } else {");
            writer.println("          vitrine.innerHTML = '<div style=\"text-align:center;padding:50px;color:#ff6b6b;font-size:18px;grid-column:1/-1;\">❌ Erro ao carregar produtos.<br><small>Tente novamente mais tarde.</small></div>';");
            writer.println("        }");
            ...
            writer.println("        const codPeca = produto.codpeca || '0000';");
            writer.println("        const descricao = produto.itemdesc || 'Produto';");
            writer.println("        const tamanho = produto.tamanho || 'Único';");
            writer.println("        const preco = typeof produto.precosug === 'string' ? parseFloat(produto.precosug).toFixed(2).replace('.', ',') : (produto.precosug || 0).toFixed(2).replace('.', ',');");
            writer.println("        const precoNumero = typeof produto.precosug === 'string' ? parseFloat(produto.precosug) : (produto.precosug || 0);");
            writer.println("        ");
            writer.println("        let imagens = [];");
            writer.println("        if (produto.imagem) {");
            writer.println("          imagens = produto.imagem.split(';').filter(img => img.trim());");
            writer.println("        }");
            writer.println("");
