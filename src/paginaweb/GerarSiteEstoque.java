...
public class GerarSiteEstoque {
    ...
    public void gerarSiteEstoque() throws ClassNotFoundException, java.sql.SQLException, InterruptedException {
        ...
        try (java.io.PrintWriter writer = new java.io.PrintWriter(caminhoArquivo, "UTF-8");
            ...
            writer.println("        ");
            writer.println("        // ✅ Verifica se a resposta é um ARRAY");
            writer.println("        if (Array.isArray(data)) {");
            ...
            writer.println("        vitrine.innerHTML = '<div style=\"text-align:center;padding:50px;color:#666;font-size:18px;grid-column:1/-1;\">🛍️ Nenhum produto disponível no momento!<br>Volte em breve!</div>';");
            ...
            writer.println("        // 🔥 Usando os nomes corretos do JSON");
            writer.println("        const codPeca = produto.id || '0000';");
            writer.println("        const descricao = produto.nome || 'Produto';");
            writer.println("        const tamanho = produto.tamanho || 'Único';");
            writer.println("        const preco = typeof produto.preco === 'string' ? parseFloat(produto.preco).toFixed(2).replace('.', ',') : (produto.preco || 0).toFixed(2).replace('.', ',');");
            writer.println("        const precoNumero = typeof produto.preco === 'string' ? parseFloat(produto.preco) : (produto.preco || 0);");
            writer.println("        ");
            writer.println("        let imagens = [];");
            writer.println("        if (produto.foto) {");
            writer.println("          imagens = produto.foto.split(';').filter(img => img.trim());");
            writer.println("        }");
            writer.println("");
            writer.println("        html += `<div class=\"card\" data-busca=\"${descricao.toLowerCase()} tam: ${tamanho.toLowerCase()} ref: ${codPeca.toLowerCase()}\" data-codigo=\"${codPeca}\">`;");
            writer.println("        html += `  <div class=\"galeria\" id=\"galeria-${codPeca}\">`;");
            writer.println("        ");
            writer.println("        if (imagens.length > 0) {");
            writer.println("          imagens.forEach((img, i) => {");
            writer.println("            html += `<img src=\"${img.trim()}\" alt=\"${descricao} - Foto ${i+1}\" class=\"slide${i === 0 ? ' ativo' : ''}\">`;");
            writer.println("          });");
            writer.println("          ");
            writer.println("          if (imagens.length > 1) {");
            writer.println("            html += `<button class=\"seta anterior\" onclick=\"mudarSlide('${codPeca}', -1)\">❮</button>`;");
            writer.println("            html += `<button class=\"seta proximo\" onclick=\"mudarSlide('${codPeca}', 1)\">❯</button>`;");
            writer.println("            html += `<div class=\"indicadores\" id=\"indicadores-${codPeca}\">`;");
            writer.println("            imagens.forEach((_, i) => {");
            writer.println("              html += `<span class=\"dot${i === 0 ? ' ativo' : ''}\" onclick=\"irParaSlide('${codPeca}', ${i})\"></span>`;");
            writer.println("            });");
            writer.println("            html += `</div>`;");
            writer.println("            html += `<span class=\"contador-fotos\">1/${imagens.length}</span>`;");
            writer.println("          }");
            writer.println("        } else {");
            ...
            writer.println("        ");
            writer.println("        html += `  </div>`;");
            writer.println("        html += `  <h3>${descricao}</h3>`;");
            writer.println("        html += `  <div class=\"info\">Tamanho: ${tamanho}</div>`;");
            writer.println("        html += `  <div class=\"codigo-item\">Código: ${codPeca}</div>`;");
            writer.println("        html += `  <div class=\"preco\">R$ ${preco}</div>`;");
            writer.println("        html += `  <button class=\"btn-add-carrinho\" onclick=\"adicionarAoCarrinho('${codPeca}', '${descricao.replace(/'/g, \"\\\\'\")}', ${precoNumero})\">`;");
            writer.println("        html += `    🛒 Adicionar ao Carrinho`;");
            writer.println("        html += `  </button>`;");
            writer.println("        html += `</div>`;");
            writer.println("      });");
            writer.println("");
            ...
            writer.println("      ");
            writer.println("      // 🔥 VERIFICA SE O ITEM JÁ ESTÁ NO CARRINHO");
            ...
            writer.println("        body: JSON.stringify({ codPeca: id })");
            writer.println("      })");
            ...
            writer.println("        }");
            writer.println("        ");
            writer.println("        // 🔥 ADICIONA COM QUANTIDADE = 1");
            ...
            writer.println("        alert('❌ Erro ao verificar disponibilidade. Tente novamente.');");
            writer.println("      });");
            writer.println("    }");
            ...
            writer.println("      loading.innerHTML = '⏳ Reservando itens...<br><small>Aguarde</small>';");
            ...
            writer.println("");
            writer.println("      console.log('📤 [RESERVA] Tentando reservar ' + itens.length + ' itens...');");
            writer.println("      console.log('📤 [RESERVA] Pedido ID:', pedidoId);");
            writer.println("");
            writer.println("      // 🔥 RESERVAR TODOS OS ITENS");
            writer.println("      let reservas = itens.map(item => {");
            writer.println("        return fetch(URL_BACKEND + '/api/pagamentos/reservar', {");
            writer.println("          method: 'POST',");
            writer.println("          headers: { 'Content-Type': 'application/json' },");
            writer.println("          body: JSON.stringify({");
            writer.println("            codPeca: item.id,");
            ...
            writer.println("        .then(response => response.json())");
            writer.println("        .then(data => {");
            writer.println("          console.log('📥 [RESERVA] Item ' + item.id + ':', data);");
            writer.println("          return { item: item, data: data };");
            writer.println("        })");
            writer.println("        .catch(error => {");
            writer.println("          console.error('❌ [RESERVA] Erro no item ' + item.id + ':', error);");
            writer.println("          return { item: item, data: { success: false, error: error.message } };");
            writer.println("        });");
            writer.println("      });");
            writer.println("");
            writer.println("      Promise.all(reservas)");
            writer.println("        .then(resultados => {");
            writer.println("          let todosReservados = resultados.every(r => r.data.success);");
            writer.println("          let itensReservados = resultados.filter(r => r.data.success).map(r => r.item.id);");
            writer.println("          let itensFalha = resultados.filter(r => !r.data.success).map(r => r.item.id);");
            writer.println("");
            writer.println("          console.log('📊 [RESERVA] Resultado:');");
            writer.println("          console.log('   ✅ Reservados:', itensReservados.join(', '));");
            writer.println("          console.log('   ❌ Falhas:', itensFalha.join(', '));");
            writer.println("");
            writer.println("          if (!todosReservados) {");
            writer.println("            document.getElementById('loadingFinal')?.remove();");
            writer.println("            alert('❌ Alguns itens não estão mais disponíveis!\\n\\n' +");
            writer.println("                  '✅ Reservados: ' + itensReservados.join(', ') + '\\n' +");
            writer.println("                  '❌ Indisponíveis: ' + itensFalha.join(', ') + '\\n\\n' +");
            writer.println("                  'Os itens indisponíveis foram removidos do seu carrinho.');");
            writer.println("");
            writer.println("            // Remove os itens que falharam do carrinho");
            writer.println("            carrinho.itens = carrinho.itens.filter(item => !itensFalha.includes(item.id));");
            writer.println("            atualizarCarrinho();");
            writer.println("");
            writer.println("            if (carrinho.itens.length === 0) {");
            writer.println("              document.getElementById('loadingFinal')?.remove();");
            ...
            writer.println("            }");
            writer.println("");
            writer.println("            // Atualiza a lista de itens para prosseguir apenas com os reservados");
            writer.println("            let itensRestantes = carrinho.itens.map(item => ({");
            writer.println("              id: item.id,");
            writer.println("              nome: item.nome,");
            writer.println("              preco: item.preco,");
            writer.println("              quantidade: 1");
            writer.println("            }));");
            writer.println("");
            writer.println("            // Recalcula o total");
            writer.println("            let novoSubtotal = itensRestantes.reduce((total, item) => total + item.preco, 0);");
            writer.println("            let novoValorTotal = novoSubtotal + carrinho.frete;");
            writer.println("");
            writer.println("            // Prossegue com os itens que foram reservados");
            writer.println("            continuarComPagamento(itensRestantes, novoSubtotal, novoValorTotal, metodo, enderecoCompleto, cep, retirarLoja, destinatario, telefone, pedidoId);");
            writer.println("            return;");
            writer.println("          }");
            writer.println("");
            writer.println("          // Todos os itens foram reservados com sucesso");
            writer.println("          console.log('✅ [RESERVA] Todos os itens reservados com sucesso!');");
            writer.println("          removerItensVitrine(itensReservados);");
            writer.println("          continuarComPagamento(itens, subtotal, valorTotal, metodo, enderecoCompleto, cep, retirarLoja, destinatario, telefone, pedidoId);");
            writer.println("        })");
            writer.println("        .catch(error => {");
            writer.println("          document.getElementById('loadingFinal')?.remove();");
            ...
            writer.println("");
            writer.println("    // ========================================");
            writer.println("    // FUNÇÃO AUXILIAR: REMOVER MÚLTIPLOS ITENS DA VITRINE");
            writer.println("    // ========================================");
            ...
            writer.println("");
            writer.println("    // ========================================");
            writer.println("    // FUNÇÃO AUXILIAR: CONTINUAR COM PAGAMENTO");
            writer.println("    // ========================================");
            writer.println("    function continuarComPagamento(itens, subtotal, valorTotal, metodo, enderecoCompleto, cep, retirarLoja, destinatario, telefone, pedidoId) {");
            ...
            writer.println("        pedidoId: pedidoId,");
            writer.println("        codPeca: itens[0]?.id || 'DESCONHECIDO'");
            writer.println("      };");
            writer.println("");
            ...
            writer.println("");
            writer.println("    // ========================================");
            writer.println("    // INICIAR - CARREGAR PRODUTOS AO ABRIR");
            writer.println("    // ========================================");
            writer.println("    document.addEventListener('DOMContentLoaded', function() {");
    ...
    // ==========================================
    // MÉTODO PARA EXECUTAR COMANDO GIT E MOSTRAR SAÍDA
    // ==========================================
    private void executarComandoGit(File diretorio, String... comando) throws IOException, InterruptedException {
        String cmdStr = String.join(" ", comando);
        System.out.println("   🔹 Executando: " + cmdStr);
        ...
        if (!hasOutput && !hasError) {
            System.out.println("   ℹ️ Comando executado sem saída.");
        }
