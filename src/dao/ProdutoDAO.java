...
import java.sql.Statement;
    ...
    public void gerarSiteEstoque() throws ClassNotFoundException, java.sql.SQLException, InterruptedException {
        ...
        try (java.io.PrintWriter writer = new java.io.PrintWriter(caminhoArquivo, "UTF-8");
            ...
            writer.println("        divEntrega.style.opacity = '0.5';");
            writer.println("        divEntrega.style.pointerEvents = 'none';");
            ...
            writer.println("      } else {");
            writer.println("        divEntrega.style.opacity = '1';");
            writer.println("        divEntrega.style.pointerEvents = 'auto';");
            ...
            writer.println("");
            writer.println("    // ========================================");
            writer.println("    // FINALIZAR COMPRA (COM NOTIFICAÇÃO PARA O SISTEMA DESKTOP)");
            writer.println("    // ========================================");
            ...
            writer.println("        alert('⚠️ Por favor, informe o nome do destinatário!\\n\\nQuem vai receber o pedido?');");
            ...
            writer.println("        enderecoCompleto = 'RETIRADA NA LOJA - ' + destinatario;");
            writer.println("        carrinho.frete = 0;");
            ...
            writer.println("        }");
            writer.println("");
            writer.println("        enderecoCompleto = 'Destinatário: ' + destinatario + ' | ';");
            ...
            writer.println("      .then(response => response.json())");
            writer.println("      .then(data => {");
            writer.println("        console.log('📥 Resposta da notificação:', data);");
            writer.println("");
            writer.println("        if (data.success && data.aprovado) {");
            writer.println("          console.log('✅ Venda aprovada pelo sistema! Gerando pagamento...');");
            writer.println("          gerarPagamento(metodo, itens, subtotal, valorTotal, cep, enderecoCompleto, destinatario, retirarLoja, pedidoId);");
            writer.println("        } else {");
            writer.println("          document.getElementById('loadingFinal')?.remove();");
            writer.println("          alert('❌ Venda não aprovada pelo sistema!\\n\\n' + (data.mensagem || 'Tente novamente.'));");
            writer.println("        }");
            writer.println("      })");
            writer.println("      .catch(error => {");
            writer.println("        document.getElementById('loadingFinal')?.remove();");
            writer.println("        console.error('❌ Erro na notificação:', error);");
            writer.println("        alert('❌ Erro ao notificar o sistema: ' + error.message + '\\n\\nTente novamente.');");
            writer.println("      });");
            writer.println("    }");
            writer.println("");
            writer.println("    // ========================================");
            writer.println("    // GERAR PAGAMENTO (APÓS APROVAÇÃO DO SISTEMA)");
            writer.println("    // ========================================");
            writer.println("    function gerarPagamento(metodo, itens, subtotal, valorTotal, cep, endereco, destinatario, retirarLoja, pedidoId) {");
            writer.println("      console.log('💰 Gerando pagamento...');");
            writer.println("");
            ...
            writer.println("");
            writer.println("      // ==========================================");
            writer.println("      // QUANDO O CLIENTE CONFIRMAR O PAGAMENTO");
            writer.println("      // ==========================================");
            writer.println("      setTimeout(() => {");
            ...
            writer.println("");
            writer.println("            let pedidoId = document.getElementById('modalDescricao').textContent.replace('Pedido #', '');");
            writer.println("            let codPeca = document.getElementById('codPecaAtual')?.value || carrinho.itens[0]?.id || 'DESCONHECIDO';");
            writer.println("            let destinatario = document.getElementById('destinatario')?.value || 'Cliente';");
            writer.println("            let telefone = document.getElementById('telefoneCliente')?.value || 'Não informado';");
            writer.println("            let enderecoCompleto = document.getElementById('enderecoCompleto')?.value || 'Não informado';");
            writer.println("            let retirarLoja = document.getElementById('chkRetirarLoja')?.checked || false;");
            writer.println("");
            writer.println("            let dadosNotificacao = {");
            ...
            writer.println("              endereco: enderecoCompleto,");
            writer.println("              retirarLoja: retirarLoja,");
            writer.println("              pedidoId: pedidoId || String(Date.now()),");
            writer.println("              itens: carrinho.itens || []");
            writer.println("            };");
            writer.println("");
            writer.println("            console.log('📤 Notificando sistema desktop...', dadosNotificacao);");
            writer.println("");
            ...
            writer.println("                statusDiv.textContent = '✅ Pagamento confirmado pela loja! 🎉';");
            writer.println("                mostrarNotificacao('✅ Venda confirmada pela loja!');");
            writer.println("");
            writer.println("                setTimeout(() => {");
            writer.println("                  fecharModalPix();");
            writer.println("                  alert('✅ Pagamento confirmado e venda registrada!\\n\\nObrigado pela compra!');");
            writer.println("                  location.reload();");
            writer.println("                }, 3000);");
            writer.println("");
            writer.println("              } else {");
            writer.println("                statusDiv.className = 'status-pagamento erro';");
            writer.println("                statusDiv.textContent = '❌ Venda não aprovada pela loja!';");
            writer.println("                alert('❌ A loja não confirmou o pagamento.\\n\\nEntre em contato para mais informações.');");
            writer.println("              }");
            writer.println("            })");
            writer.println("            .catch(error => {");
            writer.println("              console.error('❌ Erro ao notificar:', error);");
            writer.println("              statusDiv.className = 'status-pagamento erro';");
            writer.println("              statusDiv.textContent = '❌ Erro ao notificar a loja!';");
            writer.println("              alert('❌ Erro ao notificar a loja.\\n\\nTente novamente ou entre em contato.');");
            writer.println("            });");
            writer.println("          }");
            writer.println("        }");
    ...
    public void saveItem(Produto p) throws ClassNotFoundException, SQLException, Exception {
        ...
        // ==========================================
        // 1. INSERIR O PRODUTO (SEM IMAGEM)
        // ==========================================
        sql = "INSERT INTO estoque(tipoitem, codforn, nomeforn, data, itemdesc, lote, codpeca, marca, tamanho, valorpago, precosug, lucroest, perclucro, obs, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        System.out.println("Inserindo produto: " + sql);

        try {
            // Desativa auto-commit para transação
            ...
            System.out.println("✅ Produto inserido: " + linhasInseridas + " registro(s)");

            // ==========================================
            // 2. ATUALIZAR AS IMAGENS
            // ==========================================
            String codigo = p.getCodpeca();
            String imagensConcatenadas = gerarUrlsImagens(codigo);

            String sqlUpdate = "UPDATE estoque SET imagem = ? WHERE codpeca = ?";
            PreparedStatement stmtUpdate = con.prepareStatement(sqlUpdate);
            stmtUpdate.setBytes(1, imagensConcatenadas.getBytes("UTF-8"));
            stmtUpdate.setString(2, codigo);

            int linhasAtualizadas = stmtUpdate.executeUpdate();
            System.out.println("📸 Imagens atualizadas: " + linhasAtualizadas + " registro(s)");
            System.out.println("📸 URLs geradas: " + imagensConcatenadas);
            ...
            System.out.println("----------------------------------");
            System.out.println("✅ Item Salvo com sucesso na Cloud!");
            System.out.println("----------------------------------");
            ...
            if (con != null) {
                try { con.rollback(); } catch (SQLException e) { /* ignore */ }
            }
    ...
    public void saveItemCloud(Produto p) throws ClassNotFoundException, SQLException, Exception {
        ...
        try {
            ...
            stmt2.setString(15, p.getStatus());
            ...
            String sqlUpdate = "UPDATE estoque SET imagem = ? WHERE codpeca = ?";
            PreparedStatement stmtUpdate = con2.prepareStatement(sqlUpdate);
            stmtUpdate.setBytes(1, imagensConcatenadas.getBytes("UTF-8"));
            stmtUpdate.setString(2, codigo);
            ...
            // Rollback em caso de erro
            if (con2 != null) {
                try { con2.rollback(); } catch (SQLException e) { /* ignore */ }
            }
            MensagemSistema.mostrarAvisoDark(null, "Erro ao salvar o Item na Cloud: " + ex.getMessage());
            System.err.println("❌ Erro ao inserir dados: " + ex.toString());
            System.out.println("----------------------------------");
            ...
            if (con2 != null) {
                try { con2.setAutoCommit(true); } catch (SQLException e) {}
    ...
    // ==========================================
    // MÉTODO AUXILIAR PARA GERAR URLs DAS IMAGENS
    // ==========================================
    private String gerarUrlsImagens(String codigo) {
        if (codigo == null || codigo.trim().isEmpty()) {
            return "";
        }

        String baseUrl = "https://raw.githubusercontent.com/srsteinmetz12/paginawebportobella/main/fotos/";

        StringBuilder urls = new StringBuilder();
        for (int i = 1; i <= 5; i++) {
            urls.append(baseUrl).append(codigo).append("-").append(i).append(".jpg");
            if (i < 5) {
                urls.append(";");
            }
        }
    ...
    public void atualizarImagensProduto(String codigo) throws SQLException, ClassNotFoundException, UnsupportedEncodingException {
    con2 = ConnectionDB.getConnectionCloud();

        try {
            String imagensConcatenadas = gerarUrlsImagens(codigo);

            String sql = "UPDATE estoque SET imagem = ? WHERE codpeca = ?";
            PreparedStatement stmt = con2.prepareStatement(sql);
            stmt.setBytes(1, imagensConcatenadas.getBytes("UTF-8"));
            stmt.setString(2, codigo);

            int linhas = stmt.executeUpdate();
            System.out.println("📸 Imagens atualizadas para o código " + codigo + ": " + linhas + " registro(s)");
            System.out.println("📸 URLs: " + imagensConcatenadas);

            stmt.close();

        } catch (SQLException ex) {
            System.err.println("❌ Erro ao atualizar imagens: " + ex.getMessage());
            throw ex;
        } finally {
    ...
    // Método para atualizar todos os produtos de uma vez
    public void atualizarImagensTodosProdutos() throws SQLException, ClassNotFoundException, UnsupportedEncodingException {
        con2 = ConnectionDB.getConnectionCloud();

        try {
            String sqlSelect = "SELECT codpeca FROM estoque WHERE status = 'DISPONIVEL'";
            Statement stmt = con2.createStatement();
            ResultSet rs = stmt.executeQuery(sqlSelect);

            int total = 0;
            while (rs.next()) {
                String codigo = rs.getString("codpeca");
                if (codigo != null && !codigo.trim().isEmpty()) {
                    atualizarImagensProduto(codigo);
                    total++;
                }
            }

            System.out.println("✅ Total de produtos atualizados: " + total);
