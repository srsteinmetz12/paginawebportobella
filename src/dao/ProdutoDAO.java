...
import java.sql.Statement;
    ...
    public void gerarSiteEstoque() throws ClassNotFoundException, java.sql.SQLException, InterruptedException {
        ...
        try (java.io.PrintWriter writer = new java.io.PrintWriter(caminhoArquivo, "UTF-8");
            ...
            writer.println("        enderecoCompleto = 'RETIRADA NA LOJA - ' + destinatario + ' | Av. Cristóvão Colombo, 2149 - Loja 15 - Moinhos de Vento - Porto Alegre/RS';");
            writer.println("        carrinho.frete = 0;");
            ...
            writer.println("        } else {");
            writer.println("          alert('❌ Erro ao processar pedido: ' + (data.mensagem || 'Tente novamente.'));");
            writer.println("        }");
            writer.println("      })");
            writer.println("      .catch(error => {");
            writer.println("        document.getElementById('loadingFinal')?.remove();");
            writer.println("        console.error('❌ Erro na notificação:', error);");
            writer.println("        alert('❌ Erro ao processar pedido: ' + error.message + '\\n\\nTente novamente.');");
            writer.println("      });");
            writer.println("    }");
            writer.println("");
            writer.println("    // ========================================");
            ...
            writer.println("                alert('✅ Pagamento realizado com sucesso!\\n\\nA loja irá confirmar em breve.\\nObrigado pela compra!');");
            writer.println("                carrinho.itens = [];");
            writer.println("                carrinho.frete = 0;");
            writer.println("                atualizarCarrinho();");
            writer.println("                location.reload();");
